...
// TestBuildRegistrationConfigMap: data payload carries the JSON document plus
// the httpd mime-type config, and the owner ref points to the RegistrationRequest.
func TestBuildRegistrationConfigMap(t *testing.T) {
	req := &monetizeapi.RegistrationRequest{
		ObjectMeta: metav1.ObjectMeta{Name: "so-demo-registration", Namespace: "llm", UID: types.UID("rr-uid")},
	}
	doc := `{"type":"https://agents.openclaw.ai/AgentRegistration/v0.1"}`

	cm := buildRegistrationConfigMap(req, doc)

	if cm.GetKind() != "ConfigMap" {
		t.Errorf("kind = %q, want ConfigMap", cm.GetKind())
	}
	if cm.GetName() != "so-demo-registration" {
		t.Errorf("name = %q, want so-demo-registration", cm.GetName())
	}
	data, _ := cm.Object["data"].(map[string]any)
	if data["agent-registration.json"] != doc {
		t.Errorf("agent-registration.json = %v, want exact passthrough", data["agent-registration.json"])
	}
	if httpdConf, _ := data["httpd.conf"].(string); !strings.Contains(httpdConf, ".json:application/json") {
		t.Errorf("httpd.conf missing mime mapping, got %q", httpdConf)
	}
	if owners := cm.GetOwnerReferences(); len(owners) != 1 || owners[0].Kind != monetizeapi.RegistrationRequestKind {
		t.Errorf("owner refs = %+v, want RegistrationRequest owner", owners)
	}
}

// TestBuildRegistrationDeployment: content-hash annotation on pod spec triggers
// rollout when the registration document changes; busybox httpd serves /www.
func TestBuildRegistrationDeployment(t *testing.T) {
	req := &monetizeapi.RegistrationRequest{
		ObjectMeta: metav1.ObjectMeta{Name: "so-demo-registration", Namespace: "llm", UID: types.UID("rr-uid")},
		Spec:       monetizeapi.RegistrationRequestSpec{ServiceOfferName: "demo"},
	}

	dep1 := buildRegistrationDeployment(req, "hash-aaa")
	dep2 := buildRegistrationDeployment(req, "hash-bbb")

	spec1, _ := dep1.Object["spec"].(map[string]any)
	...
	if ann1["obol.org/content-hash"] != "hash-aaa" {
		t.Errorf("content-hash = %v, want hash-aaa", ann1["obol.org/content-hash"])
	}

	spec2, _ := dep2.Object["spec"].(map[string]any)
	...
	if ann2["obol.org/content-hash"] == ann1["obol.org/content-hash"] {
		t.Error("content-hash should differ between builds with different hashes")
	}

	// Label must carry the ServiceOffer name so controller ownership introspection works.
	if lbls, ok := meta1["labels"].(map[string]any); ok {
		if lbls["obol.org/serviceoffer"] != "demo" {
			t.Errorf("labels[obol.org/serviceoffer] = %v, want demo", lbls["obol.org/serviceoffer"])
		}
	} else {
		t.Error("template.metadata.labels missing")
	}

	// Deployment kind + owner reference.
	if dep1.GetKind() != "Deployment" {
		t.Errorf("kind = %q, want Deployment", dep1.GetKind())
	}
	if owners := dep1.GetOwnerReferences(); len(owners) == 0 || owners[0].UID != types.UID("rr-uid") {
		t.Errorf("owner refs = %+v, want RegistrationRequest owner uid=rr-uid", owners)
	}
}

// TestBuildRegistrationService: ClusterIP service with selector pointing at
// the per-registration deployment labels.
func TestBuildRegistrationService(t *testing.T) {
	req := &monetizeapi.RegistrationRequest{
		ObjectMeta: metav1.ObjectMeta{Name: "so-demo-registration", Namespace: "llm", UID: types.UID("rr-uid")},
	}

	svc := buildRegistrationService(req)

	if svc.GetKind() != "Service" {
		t.Errorf("kind = %q, want Service", svc.GetKind())
	}
	if svc.GetNamespace() != "llm" {
		t.Errorf("namespace = %q, want llm", svc.GetNamespace())
	}
	spec, _ := svc.Object["spec"].(map[string]any)
	if spec["type"] != "ClusterIP" {
		t.Errorf("service.type = %v, want ClusterIP", spec["type"])
	}
	selector, _ := spec["selector"].(map[string]any)
	if selector["app"] != "so-demo-registration" {
		t.Errorf("selector.app = %v, want so-demo-registration", selector["app"])
	}
	ports, _ := spec["ports"].([]any)
	if len(ports) != 1 {
		t.Fatalf("expected 1 port, got %d", len(ports))
	}
	if port, _ := ports[0].(map[string]any); port["port"] != int64(8080) {
		t.Errorf("ports[0].port = %v, want 8080", port["port"])
	}
}
