...
func validateStripeCardMinimum(price, currency string, decimals int) error {
	if strings.ToLower(strings.TrimSpace(currency)) != "usd" {
		return nil
	}
	amountMinor, err := decimalToAtomicChecked(price, decimals)
	if err != nil {
		return fmt.Errorf("invalid Stripe card amount %q", price)
	}
	n, ok := new(big.Int).SetString(amountMinor, 10)
	if !ok {
		return fmt.Errorf("invalid Stripe card amount %q", price)
	}
	if n.Cmp(big.NewInt(50)) < 0 {
		return fmt.Errorf("Stripe SPT card payments require at least 0.50 usd; got %s usd", price)
	}
	return nil
}

func decimalToAtomicChecked(amount string, decimals int) (string, error) {
	if decimals < 0 {
		return "", fmt.Errorf("invalid decimals %d", decimals)
	}
	amountFloat, _, err := new(big.Float).SetPrec(128).Parse(strings.TrimSpace(amount), 10)
	if err != nil || amountFloat == nil {
		return "", err
	}
...
func (s *stripeCardGateway) do(ctx context.Context, endpoint string, form url.Values, idempotencyKey string) (id, status string, err error) {
	...
	req.Header.Set("Stripe-Version", "2026-03-04.preview")
	if idempotencyKey != "" {
...
func (v *Verifier) serveCardGated(
	...
	authHeader := mppPaymentAuthorization(r)
	...
	if authHeader != "" {
		cred, err = parseStripeMPPCredential(authHeader, mppRealm(r), rule)
	} else {
	...
	interceptor := &settlementInterceptor{
		...
		settleFunc: func() bool {
			...
			setCardReceiptHeaders(w, paymentIntentID)
			return true
		},
