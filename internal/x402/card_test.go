...
func TestBuildCardRequirement(t *testing.T) {
	...
	if req.Extra["currency"] != "usd" || req.Extra["profileId"] != "profile_test_abc" || req.Extra["networkId"] != "profile_test_abc" {
		t.Errorf("extra = %v", req.Extra)
	}
	pmt, ok := req.Extra["paymentMethodTypes"].([]string)
	if !ok || len(pmt) != 2 || pmt[0] != "card" || pmt[1] != "link" {
		t.Errorf("extra.paymentMethodTypes = %v, want [card link]", req.Extra["paymentMethodTypes"])
	}
}
...
func TestValidateStripeCardMinimum(t *testing.T) {
	tests := []struct {
		name     string
		price    string
		currency string
		...
		{name: "usd at minimum", price: "0.50", currency: "usd", decimals: 2},
		{name: "usd above minimum", price: "1.00", currency: "USD", decimals: 2},
		{name: "non usd bypasses stripe minimum", price: "0.01", currency: "eur", decimals: 2},
		{name: "invalid usd amount", price: "not-a-number", currency: "usd", decimals: 2, wantErr: true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := validateStripeCardMinimum(tt.price, tt.currency, tt.decimals)
			if (err != nil) != tt.wantErr {
				t.Fatalf("validateStripeCardMinimum(%q, %q, %d) err=%v, wantErr=%v", tt.price, tt.currency, tt.decimals, err, tt.wantErr)
			}
		})
	}
}
...
func TestStripeCardGateway_Lifecycle(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Header.Get("Stripe-Version") != "2026-03-04.preview" {
...
func gateOnceWithAuth(gw cardGateway, guard *sptReplayGuard, auth string, proxy http.Handler) *httptest.ResponseRecorder {
	rule := cardTestRule()
	req := buildCardRequirement(rule)
	r := httptest.NewRequest(http.MethodPost, "/services/card-foo/x", nil)
...
func TestServeCardGated_NoPayment402(t *testing.T) {
	...
	if got := w.Header().Values("WWW-Authenticate"); len(got) != 1 || !strings.Contains(got[0], `method="stripe"`) {
		t.Fatalf("WWW-Authenticate = %v, want stripe Payment challenge", got)
	}
	if gw.authCalls != 0 {
...
func TestServeCardGated_AuthorizationPaymentStripe(t *testing.T) {
	t.Setenv(mppChallengeSecretEnv, "test-secret")
	rule := cardTestRule()
	challenge, err := stripeMPPChallenge(rule, "seller.example", mppChallengeSecret())
	if err != nil {
		t.Fatalf("stripeMPPChallenge: %v", err)
	}
	auth := challenge.NewCredential(map[string]any{"shared_payment_token": "spt_auth"}).ToAuthorization()
	gw := &fakeGateway{pi: "pi_auth"}
	w := gateOnceWithAuth(gw, newSPTReplayGuard(time.Hour), auth, okProxy())

	if w.Code != http.StatusOK || w.Body.String() != "upstream-ok" {
		t.Fatalf("status=%d body=%q", w.Code, w.Body.String())
	}
	if gw.authCalls != 1 || len(gw.captured) != 1 {
	...
	if _, err := mpp.ParseReceipt(w.Header().Get("Payment-Receipt")); err != nil {
		t.Fatalf("Payment-Receipt is not parseable: %v", err)
	}
}

func TestVerifierHandleProxyCardRouteUsesCardGateway(t *testing.T) {
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.WriteHeader(http.StatusOK)
		_, _ = io.WriteString(w, "card-upstream-ok")
	}))
	...
	route.StripPrefix = "/services/card-foo"
	gw := &fakeGateway{pi: "pi_proxy"}
	oldGateway, oldGuard := defaultCardGateway, defaultSPTGuard
	defaultCardGateway = gw
	defaultSPTGuard = newSPTReplayGuard(time.Hour)
	t.Cleanup(func() {
		defaultCardGateway = oldGateway
		defaultSPTGuard = oldGuard
	})
	v := newTestVerifier(t, fac.URL, []RouteRule{route})

	r := httptest.NewRequest(http.MethodPost, "/services/card-foo/v1/chat/completions", nil)
	r.Header.Set("X-PAYMENT", cardCredHeader("spt_proxy"))
	w := httptest.NewRecorder()
	v.HandleProxy(w, r)

	if w.Code != http.StatusOK || w.Body.String() != "card-upstream-ok" {
		t.Fatalf("status=%d body=%q", w.Code, w.Body.String())
	}
	if upstreamCalls != 1 || gw.authCalls != 1 || len(gw.captured) != 1 || gw.captured[0] != "pi_proxy" {
		t.Fatalf("upstream=%d auth=%d captured=%v", upstreamCalls, gw.authCalls, gw.captured)
	}
	if fac.verifyCalls.Load() != 0 || fac.settleCalls.Load() != 0 {
		t.Fatalf("card route must not use x402 facilitator: verify=%d settle=%d", fac.verifyCalls.Load(), fac.settleCalls.Load())
	}
}
