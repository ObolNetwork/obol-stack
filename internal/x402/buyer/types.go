...
// Signer produces x402 v1 payment payloads for a specific network and scheme.
// The buyer proxy holds an array of signers and selects the first one that can
// satisfy an incoming 402's requirements.
type Signer interface {
	Network() string
	Scheme() string
	CanSign(req *x402types.PaymentRequirementsV1) bool
	Sign(req *x402types.PaymentRequirementsV1) (*x402types.PaymentPayloadV1, error)
	GetPriority() int
...
type PaymentSelector interface {
	SelectAndSign(requirements []x402types.PaymentRequirementsV1, signers []Signer) (*x402types.PaymentPayloadV1, error)
}
...
func (s *DefaultPaymentSelector) SelectAndSign(requirements []x402types.PaymentRequirementsV1, signers []Signer) (*x402types.PaymentPayloadV1, error) {
