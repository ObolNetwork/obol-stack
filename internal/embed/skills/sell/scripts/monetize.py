...
def _read_upstream_auth(spec, token, ssl_ctx):
    ...
    The upstream service (LiteLLM) requires an Authorization header. The
    x402-verifier injects this via the upstreamAuth field in the pricing
    config, but only for approved (paid) requests.
    ...
    try:
        ...
        # Secret not readable (RBAC, not found, etc.) — not fatal.
...
def _add_pricing_route(spec, name, token, ssl_ctx):
    ...
    # Read upstream auth token (LiteLLM master key) so the x402-verifier can
    # inject an Authorization header on approved requests. Without this, paid
    # requests that pass x402 verification get 401 from LiteLLM.
    upstream_auth = _read_upstream_auth(spec, token, ssl_ctx)

    # Detect indentation of existing routes (match first "- pattern:" line).
    indent = ""
