...
### Integration (cast read-only queries)

Read-only registry queries use `cast call` against the contract addresses above. Write operations (registration, feedback) require a signing wallet (coming soon via the `agent-identity` skill).
...
**2. Pin to IPFS** and host the JSON at a publicly accessible URI.

**3. Register onchain** — call `register(uri)` on the IdentityRegistry contract. Write operations require a signing wallet (coming soon).
...
**5. Build reputation** — other agents/users post feedback after interacting with your agent by calling `giveFeedback()` on the ReputationRegistry.
