...
func TestServiceOfferCRD_CardPayment(t *testing.T) {
	...
	// CEL rules: payTo-when-crypto, network-when-crypto,
	// card.account-when-card, and required Tempo fields when MPP is enabled.
	rules, ok := payment["x-kubernetes-validations"].([]any)
	if !ok {
		t.Fatal("payment.x-kubernetes-validations missing")
	}
	if len(rules) != 4 {
		t.Fatalf("payment x-kubernetes-validations count = %d, want 4", len(rules))
	}
	joined := ""
	for _, r := range rules {
		joined += r.(map[string]any)["rule"].(string) + "\n"
	}
	for _, want := range []string{"self.payTo", "self.network", "self.card.account", "self.mpp.tempo.payTo", "self.mpp.tempo.asset"} {
