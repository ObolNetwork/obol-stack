...
type PaymentTerms struct {
	...
	// MPP holds optional HTTP-auth payment rails advertised alongside the
	// primary method.
	MPP *MPPTerms `json:"mpp,omitempty" yaml:"mpp,omitempty"`
...
type CardTerms struct {
	...
	// ProfileID is the preferred Stripe profile id (profile_... or
	// profile_test_...) surfaced in the MPP challenge.
	...
	// the client. Defaults to ["card","link"] at the gateway when empty.
	PaymentMethodTypes []string `json:"paymentMethodTypes,omitempty" yaml:"paymentMethodTypes,omitempty"`
}

// MPPTerms groups optional MPP HTTP-auth payment rails.
type MPPTerms struct {
	Tempo *TempoMPPTerms `json:"tempo,omitempty" yaml:"tempo,omitempty"`
}

// TempoMPPTerms advertises a Tempo MPP pull-payment rail.
type TempoMPPTerms struct {
	...
	ChainID  int64  `json:"chainId,omitempty" yaml:"chainId,omitempty"`
