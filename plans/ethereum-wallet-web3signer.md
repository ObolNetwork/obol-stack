...
> **Status**: Implemented — Phase 1-3 complete, Phase 4 in progress
...
```
1. obol agent init
2. Go code: secp256k1.GeneratePrivateKey() → private key (decred/secp256k1)
3. Go code: derive public key → derive Ethereum address (keccak256)
4. Go code: write YAML key config → $DATA_DIR/openclaw-<id>/storage-web3signer-0/keys/<keyid>.yaml
5. helmfile sync → deploys web3signer with PVC mounted at key directory
6. Go code: create ConfigMap web3signer-metadata with address + public key (post-sync)
7. web3signer starts → reads key files → ready to sign
```
...
Generated by `generateWeb3SignerValues()` in `internal/openclaw/web3signer.go`:
...
image:
  ...
  tag: "25.12.0"    # Pin a specific stable version

# Override the default command to use eth1 mode instead of eth2.
# The chart's _cmd.tpl hardcodes "eth2" — we need "eth1" for SECP256K1 signing.
config: |
  data-path: "/data/keys"
...
# Slashing protection DB (PostgreSQL) not needed for ETH1 file-based keys.
...
```
Host path: $DATA_DIR/openclaw-<id>/storage-web3signer-0/keys/
...
**File layout on host**:
```
$DATA_DIR/openclaw-<id>/storage-web3signer-0/keys/
└── <keyid>.yaml         # Web3Signer YAML key config (v25+ format)
```

**YAML key config format** (Web3Signer v25+ inline format):
```yaml
type: "file-raw"
keyType: "SECP256K1"
...
### 3.4 Key Generation in Go (Implemented)

Implemented in `internal/openclaw/web3signer.go` using `github.com/decred/dcrd/dcrec/secp256k1/v4`:
...
**Decision**: Used `decred/secp256k1` — lightweight, no transitive dependency bloat from go-ethereum.
...
### Phase 1: Key Generation & Web3Signer Deployment -- DONE

**File**: `internal/openclaw/web3signer.go`

**Implemented Functions**:
- `GenerateSigningKey() (*Web3SignerKey, error)` — uses `decred/secp256k1`
- `ProvisionKeyFiles(keysDir string, key *Web3SignerKey, label string) error` — writes `.yaml` inline key to host-path PVC
- `generateWeb3SignerValues(id string) string` — produces `values-web3signer.yaml`
- `ApplyMetadataConfigMap(cfg, id string, key *Web3SignerKey) error` — applies `web3signer-metadata` ConfigMap via kubectl
- `Web3SignerKeysPath(cfg, id string) string` — returns `$DATA_DIR/openclaw-<id>/storage-web3signer-0/keys/`
- `ensureWeb3Signer(cfg, id, deploymentDir string)` — idempotent provisioning for existing deployments
- `applyWeb3SignerMetadata(cfg, id string)` — reads key YAML, derives address, creates ConfigMap post-sync
- `extractPrivateKeyFromYAML(content string) string` — parses inline privateKey from YAML key file

**Modified**: `internal/openclaw/openclaw.go`
- `generateHelmfile()` — adds `ethereum` repo and `web3signer` release
- `Onboard()` — calls key generation + provisioning before `doSync()`
- `doSync()` — calls `applyWeb3SignerMetadata()` after helmfile sync to create ConfigMap
...
### Phase 2: Skill Script — Accounts & Signing -- DONE

**Files created**:
- `internal/embed/skills/ethereum-wallet/scripts/signer.py` (293 lines)
...
### Phase 3: Transaction Signing & Submission -- DONE

**Commands implemented** in `scripts/signer.py`:
...
### Phase 4: Skill Documentation & Polish -- IN PROGRESS

**Done**:
- `internal/embed/skills/ethereum-wallet/SKILL.md` — complete skill documentation
- `internal/openclaw/web3signer_test.go` — unit tests for key generation, values generation, YAML format, ConfigMap metadata, helmfile generation, key path, JSON indentation, YAML private key extraction
- `internal/openclaw/skills_injection_test.go` — updated for renamed skills

**Remaining**:
- Integration test in `internal/openclaw/integration_test.go` — full deploy + sign flow
- `ethereum-networks` SKILL.md cross-reference to wallet skill
...
| Q7 | secp256k1 in Go | Using `github.com/decred/dcrd/dcrec/secp256k1/v4` | Lightweight, no go-ethereum transitive dependency bloat. |

---

## 8. Testing Strategy

### Unit Tests (`internal/openclaw/web3signer_test.go`) -- ALL PASSING

| Test | Validates |
|------|-----------|
| `TestGenerateSigningKey` | Valid secp256k1 key, correct address derivation, deterministic length |
| `TestGenerateSigningKey_Unique` | Two generated keys are distinct |
| `TestProvisionKeyFiles` | `.yaml` file has correct structure, 0600 permissions, inline privateKey |
| `TestProvisionKeyFiles_CreatesDirectory` | Nested directory creation works |
| `TestGenerateWeb3SignerValues` | Valid YAML, ETH1 via customCommand, slashingprotectiondb disabled, ClusterIP |
| `TestMetadataPayload_JSON` | Correct JSON structure, marshal/unmarshal round-trip |
| `TestGenerateHelmfile_IncludesWeb3Signer` | Two releases (openclaw + web3signer), same namespace, ethereum repo |
| `TestWeb3SignerKeysPath` | Correct host-side path construction |
| `TestExtractPrivateKeyFromYAML` | Parses privateKey from YAML key files (standard, single-quote, missing, empty) |
| `TestIndentJSON` | JSON indentation for YAML embedding |
...
| `consensys/web3signer` Docker image | 25.12.0 | Docker Hub | Pin specific version |
| `github.com/decred/dcrd/dcrec/secp256k1/v4` | v4 | Go module | Lightweight secp256k1 for key generation |
