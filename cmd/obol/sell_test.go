...
func TestSellHTTP_Flags(t *testing.T) {
	...
	requireFlags(t, flags,
		...
		"pay-with", "stripe-account", "card-currency", "stripe-profile-id",
	)
...
func runCardResolve(t *testing.T, args ...string) (map[string]any, error) {
	...
	cmd := &cli.Command{
		...
		Flags: []cli.Flag{
			...
			&cli.StringFlag{Name: "stripe-profile-id"},
...
func TestResolveCardPayment_Valid(t *testing.T) {
	out, err := runCardResolve(t, "--stripe-account", "acct_1A2b3C4d", "--card-currency", "eur", "--stripe-profile-id", "profile_test_cli")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if card["profileId"] != "profile_test_cli" {
		t.Errorf("card.profileId = %v, want profile_test_cli", card["profileId"])
	}
