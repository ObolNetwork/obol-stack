...
" 2>&1; then
    pass "Prometheus x402-buyer target: up"
else
...
if [ "$running" -gt 0 ] && [ "$running" = "$total" ]; then
    pass "Monitoring namespace: $running/$total pods running"
else
...
step "Prometheus monitoring ready"
prom_out=$("$OBOL" kubectl get --raw \
    /api/v1/namespaces/monitoring/services/monitoring-kube-prometheus-prometheus:9090/proxy/-/ready \
    2>&1) || true
if echo "$prom_out" | grep -q "Ready"; then
    pass "Prometheus Server is Ready"
else
    fail "Prometheus not ready — ${prom_out:0:100}"
fi
