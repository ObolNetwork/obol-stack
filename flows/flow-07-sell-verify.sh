...
# §2.1 pre-check: obol-skill-md deployment serving /skill.md (created by heartbeat)
# The monetize agent's _publish_skill_md creates this busybox httpd deployment.
# It serves the machine-readable service catalog at /skill.md.
run_step_grep "obol-skill-md pod running" "Running" \
    "$OBOL" kubectl get pods -n openclaw-obol-agent --no-headers
...
# §1.4 Pricing Config: x402-pricing ConfigMap has active route for flow-qwen
# (monetize §1.4 and Pricing Configuration section show the routes[] structure)
step "x402-pricing ConfigMap has active route for /services/flow-qwen"
pricing_yaml=$("$OBOL" kubectl get cm x402-pricing -n x402 \
    -o jsonpath='{.data.pricing\.yaml}' 2>&1) || true
if echo "$pricing_yaml" | grep -q "flow-qwen\|services/flow-qwen"; then
    pass "x402-pricing has route for /services/flow-qwen"
else
    fail "x402-pricing missing route for flow-qwen — ${pricing_yaml:0:200}"
fi
