...
if echo "$skill_out" | grep -q "x402\|service\|obol"; then
    ...
    fail "Service catalog not found — ${skill_out:0:200}"
fi
...
if echo "$skill_out" | grep -q "flow-qwen"; then
    ...
    # May not be listed if heartbeat hasn't run yet since SO was deleted+recreated
    fail "/skill.md does not list flow-qwen — ${skill_out:0:200}"
fi
...
CONSUMER_KEY  = os.environ["CONSUMER_PRIVATE_KEY"]
...
body = {"model": "qwen3:0.6b", "messages": [{"role": "user", "content": "What is 2+2?"}], "max_tokens": 20}
