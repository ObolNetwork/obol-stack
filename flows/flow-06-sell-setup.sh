...
# obol sell http may restart the openclaw pod (new tunnel URL → helmfile sync).
# Wait for the pod to be ready before starting the heartbeat poll, so we
# start counting from a stable state (not mid-restart).
step "openclaw pod ready after sell http"
"$OBOL" kubectl rollout status deployment/openclaw \
    -n openclaw-obol-agent --timeout=120s 2>/dev/null \
    && pass "openclaw pod ready" \
    || fail "openclaw pod not ready after 120s"
