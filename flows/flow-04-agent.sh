...
# §4: HEARTBEAT.md injected into agent workspace (agent init output confirms this)
# obol agent init injects HEARTBEAT.md with the monetize reconcile command
step "HEARTBEAT.md injected into agent workspace"
HEARTBEAT_FILE="$OBOL_DATA_DIR/openclaw-obol-agent/openclaw-data/.openclaw/workspace/HEARTBEAT.md"
if [ -f "$HEARTBEAT_FILE" ] && grep -q "monetize\|python3\|sell" "$HEARTBEAT_FILE" 2>/dev/null; then
    pass "HEARTBEAT.md injected with monetize reconcile command"
else
    fail "HEARTBEAT.md missing or empty at $HEARTBEAT_FILE"
fi
...
# §4: Verify openclaw config — heartbeat and model settings (agent init patches these)
# obol agent init calls ensureHeartbeatActive() which patches openclaw-config ConfigMap
oc_config=$("$OBOL" kubectl get cm openclaw-config -n openclaw-obol-agent \
    -o jsonpath='{.data.openclaw\.json}' 2>&1) || true

step "Agent heartbeat configured to 5m interval"
if echo "$oc_config" | python3 -c "
import sys, json
d = json.load(sys.stdin)
heartbeat = d.get('agents', {}).get('defaults', {}).get('heartbeat', {})
every = heartbeat.get('every', '')
assert every == '5m', f'expected 5m, got {every!r}'
print(f'Heartbeat: every={every}')
" 2>&1; then
    pass "Heartbeat configured to 5m (agent will reconcile every 5 minutes)"
else
    fail "Heartbeat not configured to 5m — ${oc_config:0:200}"
fi

# §4: "A heartbeat that runs the agent periodically" includes the agent model config
...
# §4 RBAC: Both monetize ClusterRoles must exist (deployed by k3s manifests at stack up)
step "RBAC: monetize ClusterRoles exist"
cr_read=$("$OBOL" kubectl get clusterrole openclaw-monetize-read 2>&1) || true
cr_workload=$("$OBOL" kubectl get clusterrole openclaw-monetize-workload 2>&1) || true
if echo "$cr_read" | grep -q "openclaw-monetize-read" && \
   echo "$cr_workload" | grep -q "openclaw-monetize-workload"; then
    pass "ClusterRoles: openclaw-monetize-read + openclaw-monetize-workload"
else
    fail "Missing monetize ClusterRole(s)"
fi

# §4 RBAC: workload ClusterRole allows CRUD on ServiceOffers (obol.org)
step "RBAC: openclaw-monetize-workload can CRUD ServiceOffers"
workload_rules=$("$OBOL" kubectl get clusterrole openclaw-monetize-workload \
    -o jsonpath='{.rules}' 2>&1) || true
if echo "$workload_rules" | python3 -c "
import sys, json
...
" 2>&1; then
    pass "openclaw-monetize-workload can CRUD ServiceOffers (obol.org)"
else
    fail "RBAC workload rule missing ServiceOffer CRUD — ${workload_rules:0:100}"
fi

# §4: RBAC bindings — obol agent init patches ClusterRoleBindings (troubleshooting §RBAC)
# Both monetize ClusterRoleBindings must include openclaw SA as a subject.
...
rbac_workload=$("$OBOL" kubectl get clusterrolebinding openclaw-monetize-workload-binding \
    -o jsonpath='{.subjects}' 2>&1) || true
if echo "$rbac_out" | grep -q "openclaw" && echo "$rbac_workload" | grep -q "openclaw"; then
    pass "Both monetize ClusterRoleBindings have openclaw SA"
else
    fail "ClusterRoleBinding missing openclaw SA — read: ${rbac_out:0:50} workload: ${rbac_workload:0:50}"
fi
